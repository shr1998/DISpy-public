...
..
    PyTorch tensor adaptation for forward calculation and inversion.
"""
import numpy as np
import torch

from _surf96 import surf96
from utillis_new_gpu import numpy2tensor
...
def dltar1_vector(vc, t, d, alpha, beta, rho, llw,device):
    """
...
def dltar4_vector(vc, t0, d, alpha, beta, rho, llw=1,device="cpu"):
    """
    Rayleigh-wave period equation.
    ----------
    input: 
        vc: phase velocity list of observation : 1D list
        t : period list of observatygtion : 1D list
        d : the thickness : 1D list
        alpha : vp : 1D list
        beta : vs : 1D list
        rho : density : 1D list
        llw : sign for mark contain water layer or not : 0 for contain water layer
    """
    vc = numpy2tensor(vc).to(device)
    t0 = numpy2tensor(t0).to(device)
    d = numpy2tensor(d).to(device)
    omega = twopi/t0
    wvno = 1/vc * omega

    # Preallocate Dunkin's matrix
    ca = torch.empty((vc.shape[-1],5, 5)).to(device)
    ...
    for m in range(len(d) - 2, llw, -1):
        ...
        # 多层累计
        ee = torch.zeros((wvno.shape[-1],1,5)).to(device)
        e_temp = e[:,[m+1],:]
