...
def dltar4_vector(pvs_lists, t_lists, d, alpha, beta, rho, llw=1, device="cpu"):
    ...
    omega = twopi / t_lists
    wvno = (1 / pvs_lists * omega).to(device)

    # Preallocate Dunkin's matrix [station,pvs_points,5,5]
    ca = torch.empty((pvs_lists.shape[0], pvs_lists.shape[1], 5, 5)).to(device)

    # dunkin's matrix [station,pvs_points,vs,5]
    e = torch.zeros((wvno.shape[0], wvno.shape[1], alpha.shape[1], 5)).to(device)
