...
..
    NumPy adaptation for forward calculation and inversion.
"""
import numpy as np
...
try:
    from utillis_new_gpu import numpy2tensor as _numpy2tensor
except Exception:
    _numpy2tensor = None


def numpy2tensor(x):
    if _numpy2tensor is not None and torch is not None:
        return _numpy2tensor(x)
...
def normc_vector(ee, axis=-1):
    """
    Normalize Haskell or Dunkin vectors.
    """
    ee = np.asarray(ee)
    t1 = np.max(np.abs(ee), axis=axis, keepdims=True)
    t1 = np.where(t1 < 1.0e-40, 1.0, t1)
    ee = ee / t1
    ex = np.log(t1)
    return ee, ex
...
def dltar_vector(vc, t, d, alpha, beta, rho, ifunc, llw, device="cpu"):
    """
    ...
    """
    if ifunc == 1:
        return dltar1_vector(vc, t, d, alpha, beta, rho, llw, device)
    elif ifunc == 2:
        return dltar4_vector(vc, t, d, alpha, beta, rho, llw, device)
    else:
        return fast_delta_vector(vc, t, d, alpha, beta, rho, llw, device)
...
def dltar4_vector(pvs_lists, t_lists, d, alpha, beta, rho, llw=1, device="cpu"):
    ...
    for m in range(d.shape[-1] - 2, llw, -1):
        ...
        # 多层累计
        # ee = np.zeros(ca.shape[0], ca.shape[1], 1, 5).to(device)  # [m,n,1,5]
        e_temp = e[:, :, [m + 1], :]
        # ee = np.bmm(e_temp,ca)
