...
def dltar4_vector(pvs_lists, t_lists, d, alpha, beta, rho, llw=-1, device="cpu"):
    """
    Rayleigh-wave period equation.
    ----------
    input:
    ...
    ca = np.empty((pvs_lists.shape[0], pvs_lists.shape[1], 5, 5))

    # dunkin's matrix [station,pvs_points,vs,5]
    e = np.zeros((wvno.shape[0], wvno.shape[1], alpha.shape[1], 5))
    omega = np.maximum(omega, 1.0e-4)
    wvno2 = wvno * wvno
    xka = omega / alpha[:, -1]  # 修改为直接索引最后一列
    xkb = omega / beta[:, -1]
    wvnop = wvno + xka
    wvnom = np.abs(wvno - xka)
    ra = np.sqrt(wvnop * wvnom)
    ...
    for m in range(d.shape[-1] - 2, llw, -1):
        ...
        dpth = np.ones_like(wvno) * d[:, m]
        rho1 = rho[:, m]
        p = ra * dpth
        q = rb * dpth
        # 省略一些中间的计算
        e_temp = e[:, :, m + 1, :]
        ee = np.matmul(e_temp, ca)
...
def var_vector(p, q, ra, rb, wvno, xka, xkb, dpth):
    ...
    # Examine P-wave eigenfunctions
    # Checking whether c > vp, c = vp or c < vp
    pex = np.zeros_like(wvno)
    fac = np.zeros_like(wvno)
    sinp = np.zeros_like(wvno)
    w = np.zeros_like(wvno)
    x = np.zeros_like(wvno)
    cosp = np.zeros_like(wvno)
    # wvno < xka
    mask = wvno < xka
    sinp[mask] = np.sin(p[mask])
    w[mask] = sinp[mask] / ra[mask]
    x[mask] = -ra[mask] * sinp[mask]
    cosp[mask] = np.cos(p[mask])

    # wvno > xka
    mask = wvno > xka
    pex[mask] = p[mask]
    mask1 = (wvno > xka) & (p < 16.0)
    fac[mask1] = np.exp(-2.0 * p[mask1])
    ...
    # Examine S-wave eigenfunctions
    # Checking whether c > vs, c = vs or c < vs
    sex = np.zeros_like(wvno)
    fac = np.zeros_like(wvno)
    sinq = np.zeros_like(wvno)
    y = np.zeros_like(wvno)
    z = np.zeros_like(wvno)
    cosq = np.zeros_like(wvno)
    # wvno < xkb
    mask = wvno < xkb
    sinq[mask] = np.sin(q[mask])
    y[mask] = sinq[mask] / rb[mask]
    z[mask] = -rb[mask] * sinq[mask]
    cosq[mask] = np.cos(q[mask])

    # wvno > xkb
    mask = wvno > xkb
    sex[mask] = q[mask]
    mask1 = (wvno > xkb) & (q < 16.0)  # 同时满足wvno>xkb和q<16.0的条件
    fac[mask1] = np.exp(-2.0 * q[mask1])
    ...
    # Form eigenfunction products for use with compound matrices
    a0 = np.zeros_like(wvno)
    exa = pex + sex
    a0[exa < 60.0] = np.exp(-exa[exa < 60.0])
    cpcq = cosp * cosq
    cpy = cosp * y
    cpz = cosp * z
    cqw = cosq * w
    ...
    # fac = np.zeros_like(wvno)
    # qmp = sex - pex
    # fac[qmp>-40.0] = np.exp(qmp[qmp>-40.0])
    # cosq *= fac
    # y *= fac
    # z *= fac
...
def fast_delta(wvno, omega, d, alpha, beta, rho, llw):
    ...
    r = np.zeros(mmax, dtype=np.complex_)
    s = np.zeros(mmax, dtype=np.complex_)
    Ca = np.ones(mmax - 1, dtype=np.complex_)
    Cb = np.ones(mmax - 1, dtype=np.complex_)
    Sa = np.zeros(mmax - 1, dtype=np.complex_)
    Sb = np.zeros(mmax - 1, dtype=np.complex_)

    eps = np.zeros(mmax - 1, dtype=np.complex_)
    eta = np.zeros(mmax - 1, dtype=np.complex_)
    a = np.zeros(mmax - 1, dtype=np.complex_)
    ap = np.zeros(mmax - 1, dtype=np.complex_)
    b = np.zeros(mmax - 1, dtype=np.complex_)
    bp = np.zeros(mmax - 1, dtype=np.complex_)
    scale = np.zeros(mmax - 1, dtype=np.int32)
    ...
    for i in range(mmax):
        ...
        if c < alpha[i]:
            r[i] = np.sqrt(1.0 - c2 / alpha[i] ** 2)
        elif c > alpha[i]:
            r[i] = np.sqrt(c2 / alpha[i] ** 2 - 1.0) * 1j

        if c < beta[i]:
            s[i] = np.sqrt(1.0 - c2 / beta[i] ** 2)
        elif c > beta[i]:
            s[i] = np.sqrt(c2 / beta[i] ** 2 - 1.0) * 1j
    ...
    for i in range(mmax - 1):
        ...
        if c < alpha[i]:
            Ca[i] = np.cosh(wvno * r[i] * d[i])
            Sa[i] = np.sinh(wvno * r[i] * d[i])
        elif c > alpha[i]:
            Ca[i] = np.cos(wvno * r[i].imag * d[i])
            Sa[i] = np.sin(wvno * r[i].imag * d[i]) * 1j

        if c < beta[i]:
            Cb[i] = np.cosh(wvno * s[i] * d[i])
            Sb[i] = np.sinh(wvno * s[i] * d[i])
            ...
            Cb[i] = np.cos(wvno * s[i].imag * d[i])
            Sb[i] = np.sin(wvno * s[i].imag * d[i]) * 1j
    ...
    return np.real(X[1] + s[-1] * X[3] - r[-1] * (X[3] + s[-1] * X[4]))
        ...
        for k in range(ibeg, iend):
            ...
            if iret:
                if iq > 0:
                    # 第一次失败 → 放宽条件再试一次
                    c1 = clow
                    # c1_new = c[k - 1] if k > 0 else cm  # 用前值或半空间值作初值
                    #
                    # c1_new, del1st_new, iret_new = getsol(
                    #     t[k], c1_new, clow_new, dc * 2, cm, betmx, ifirst, del1st,
                    #     d, a, b, rho, ifunc, llw
                    # )
                    # if not iret_new:
                    #     c1 = c1_new
                    # else:
                    #     # 还是失败 → 设 nan 或继承
                    #     c1 = c[k - 1] if k > 0 and not np.isnan(c[k - 1]) else np.nan
                    ...
                    raise Exception("failed to find root for fundamental mode")
